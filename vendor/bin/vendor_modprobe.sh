...
do
	...
	# Keep hyp_core_ctl unloaded to avoid boot-time CPU5/CPU6 isolation.
	load_modules=$(cat ${dir}/modules.load | \
		grep -w -v ${blocklist_expr} | \
		grep -vw "hyp_core_ctl" | \
		grep -vw "hyp_core_ctl.ko")
