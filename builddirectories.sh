#!/bin/bash
# Recreate the vendor/ empty-directory skeleton before packing (git cannot track
# empty dirs). Generated from the curated vendor/ tree so every directory present
# after checkout is restored; mkdir -p is idempotent for dirs that hold files.
mkdir -p vendor/app
mkdir -p vendor/bin
